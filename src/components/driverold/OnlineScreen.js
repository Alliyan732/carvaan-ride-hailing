    ...
    const interval = setInterval(()=>
  {
    (async ()  => {
      try{
        const querySnapshot = await getDocs(collection(db, "Rides"));
        querySnapshot.forEach(async (doc1) => {
          console.log('Inside Claer Interval');
          
          if( rider_id !== '' && doc1.data().RiderAccepted == rider_id  ){
            {
              storeDataAsyncStorage("RIDE_ACCEPTED",(doc1.id))
              storeDataAsyncStorage("ORIGIN",(doc1.data().Origin))
              storeDataAsyncStorage("CUST_ID",(doc1.data().CustomerId))
              storeDataAsyncStorage("DESTINATION",doc1.data().Destination)
              storeDataAsyncStorage("FARE_COST",doc1.data().Cost.FareCost)
              storeDataAsyncStorage("TAX_COST",doc1.data().Cost.TaxCost)
              storeDataAsyncStorage("TOTAL_COST",doc1.data().Cost.TotalCost)
              // storeDataAsyncStorage("DISTANCE",doc1.data().Distance)
              // storeDataAsyncStorage("DURATION",doc1.data().Duration)
              console.log('Ride Data Added to local storage ',doc1.data().Origin,doc1.data().Destination);
              navigation.navigate('RideRequest')
